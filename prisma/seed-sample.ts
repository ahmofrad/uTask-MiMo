...
async function main() {
  ...
  // Date-only fields (due dates) use canonical UTC day markers — the app's
  // stored convention is starts at `00:00:00.000Z` / dues at `23:59:59.999Z` —
  // so fresh installs and CI never create the zone-local midnights the app
  // had to be hardened against. `day()` stays for real instants (completedAt).
  const dueDay = (d: number) =>
    new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + d, 23, 59, 59, 999));
