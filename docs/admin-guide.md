...
2. Fill in:
   ...
   - **Search Filter**: e.g. `(&(objectClass=person)(uid={{username}}))`
   - **Attribute Mapping**: map LDAP attributes (uid, mail, cn) to user fields.
3. Click **Test Connection** to verify.
4. Toggle **Enable LDAP** to activate.

Users are created on first login (JIT provisioning). Groups can be imported via the sync button.
...
- Due-soon reminders (24 h before due date)
