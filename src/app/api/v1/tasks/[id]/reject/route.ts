...
import { isTaskFinalizer, TaskNotPendingApprovalError, rejectTask, toPlainTaskRow } from "@/lib/tasks";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = approvalDecisionSchema.safeParse((await readJsonBody(request)) ?? {});
  if (!parsed.success) {
    return NextResponse.json(validationError(parsed.error), { status: 400 });
  }
  // A rejection without a reason is meaningless — require one.
  ...
  let result: Awaited<ReturnType<typeof rejectTask>>;
  try {
    result = await rejectTask(resolvedParams.id, userId, parsed.data.reason);
  } catch (err) {
